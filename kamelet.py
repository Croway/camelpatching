...
parser.add_argument('endpoint', nargs='?', default='',
                    help='Endpoint to stop at (endbeforedelete, endbeforepre, endbeforepost)')
...
print_header("Camel Kamelets Patching Script")
...
print_step(1, 7, "Cleaning up directories")
run_command(["rm", "-rf", dir_name], f"Removing {dir_name}")
run_command(["rm", "-rf", prodlocation], f"Removing {prodlocation}")

# Clone
print_step(2, 7, "Cloning repositories")
run_command(["git", "clone", "git@github.com:jboss-fuse/camel-kamelets.git", dir_name],
            f"Cloning jboss-fuse/camel-kamelets into {dir_name}")
run_command(["git", "clone", "-b", currentprodbranch, "git@github.com:jboss-fuse/camel-kamelets.git", prodlocation],
            f"Cloning {currentprodbranch} into {prodlocation}")

print_step(3, 7, "Setting up git remotes and fetching")
os.chdir(dir_name)
run_command(["git", "remote", "add", "upstream", "git@github.com:apache/camel-kamelets.git"],
...
if "endbeforedelete" in endpoint:
    print_success("Stopping before file deletion (endbeforedelete)")
    sys.exit(0)

# Delete unsupported files
print_step(5, 7, "Deleting unsupported files")
delete_file = Path(f"../{patchdir}/files.delete")
if delete_file.exists():
    print_info("Deleting files listed in files.delete")
    with open(delete_file, 'r') as f:
        for line in f:
            line = line.strip()
            if line:
                console.print(f"  [dim]→ Deleting {line}[/dim]")
                subprocess.run(["git", "rm", line], capture_output=True)

    time.sleep(3)
    run_command(["git", "commit", "-a", "-m", "Delete unsupported files"],
                "Committing file deletions")
else:
    console.print("  [yellow]No files.delete found[/yellow]")
...
run_command(["git", "commit", "-a", "-m", "Refresh"],
            "Committing build refresh")
...
print_step(6, 7, "Applying post-prod-maven-plugin patches")
...
run_command([
    ...
    "clean",
    "install"
], "Running final mvn clean install", env=os.environ)

console.print("\n")

# Print summary
if failed_patches or partial_patches:
    print_header("⚠ Camel Kamelets Patching Complete with Issues")
    console.print(f"[bold yellow]Built Camel Kamelets {vers} but some patches need attention[/bold yellow]\n")
    ...
    print_header("✓ Camel Kamelets Patching Complete!")
    console.print(f"[bold green]Successfully built Camel Kamelets {vers}[/bold green]")
